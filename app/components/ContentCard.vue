...
const marked = computed(() => (props.section === 'personnages' || props.section === 'ennemis')
  && isMarked(characterStatusId(props.section === 'ennemis' ? 'enemies' : 'npcs', props.entry.id)));
...
<template>
  <article :class="$style.card">
    ...
    <h3 :class="marked && $style.marked"><NuxtLink :to="`/${section}/${entry.id}`">{{ name }}</NuxtLink></h3>
    <p><em>{{ meta }}</em></p>
