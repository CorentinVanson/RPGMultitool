...
const {
  ...
  dailyIncome, dailyVisitors, populationGrowth, isBuilt, build, demolish, assignNpc, advanceDay, addGold, addPopulation,
} = useCampoFrontiera();
...
const treeLevels = computed(() => Array.from({ length: 5 }, (_, level) => ({ level, constructions: CAMPO_CONSTRUCTIONS.filter((construction) => construction.level === level) })));
...
<template>
  <section>
    ...
    <section :class="$style.treeSection">
      <div :class="$style.sectionHeader"><div><h2>Arbre de constructions</h2><p :class="$style.muted">Les tentes et la palissade sont déjà là. Les joueurs choisissent ensuite leurs priorités et leurs responsables.</p></div></div>
      <div :class="$style.tree">
        ...
        <article v-for="construction in row.constructions" :key="construction.id" :class="[$style.node, isBuilt(construction.id) && $style.built, !isBuilt(construction.id) && !availableIds.has(construction.id) && $style.locked]">
          ...
          <p v-if="construction.cost" :class="$style.cost"><strong>{{ construction.cost }} pièces</strong> · +{{ construction.baseIncome }} pièces/jour · +{{ construction.baseVisitors }} visiteur{{ construction.baseVisitors > 1 ? 's' : '' }}/jour</p>
