...
const characterDraft = reactive<Omit<PlayerCharacter, 'id'>>({
  name: '', species: '', className: '', level: '', background: '',
...
function addPlayerCharacter() {
  ...
  addCharacter(activeTeam.value.id, {
    ...
    className: characterDraft.className.trim(), level: characterDraft.level.trim(),
...
function resetCharacterDraft() {
  characterDraft.name = ''; characterDraft.species = ''; characterDraft.className = '';
  ...
  <section>
    ...
    <div v-if="teams.length" :class="$style.layout">
      ...
      <div v-if="activeTeam" :class="$style.content">
        <header :class="$style.teamHeader"><div><p :class="$style.eyebrow">Équipe active</p><h2>{{ activeTeam.name }}</h2></div><button type="button" :class="$style.danger" @click="removeActiveTeam">Supprimer</button></header>
        ...
        <section :class="$style.sheetSection">
          <h3>Ajouter un personnage joueur</h3>
          <form :class="$style.characterForm" @submit.prevent="addPlayerCharacter">
            <div :class="$style.formGrid">
              ...
              <label>Espèce<input v-model="characterDraft.species" type="text" maxlength="80" placeholder="Ex. Licae — volpi rosse"></label>
            ...
            <button type="submit" :disabled="!characterDraft.name.trim()">Ajouter la fiche</button>
          </form>
        </section>
        ...
        <section><h3>Fiches de l’équipe</h3><p v-if="!activeTeam.characters.length" :class="$style.hint">Aucun personnage joueur n’a encore été ajouté.</p><div v-else :class="$style.characterGrid">
          <article v-for="character in activeTeam.characters" :key="character.id" :class="$style.characterCard">
            <header :class="$style.cardHeader"><div><h4>{{ character.name }}</h4><p>{{ [character.species, character.className, character.level && `Niveau ${character.level}`].filter(Boolean).join(' · ') || 'Fiche sans détails' }}</p></div><button type="button" :class="$style.miniDanger" title="Supprimer cette fiche" @click="deleteCharacter(activeTeam.id, character.id)">Supprimer</button></header>
...
input, textarea { min-width: 0; background: var(--bg); color: var(--text); border: 1px solid #4a3a28; border-radius: 4px; padding: .55rem .65rem; font: inherit; }
