---
name: "Sicario del Tralcio"
type: "Humanoïde (thérien), légalement mauvais"
cr: "2 (450 XP)"
image: "/images/enemies/sicario-del-tralcio.svg"
ac: 14
hp: "33 (6d8+6)"
speed: "12 m"
stats:
  for: 12
  dex: 16
  con: 13
  int: 12
  sag: 11
  cha: 10
summary: "Assassin à la solde de la Compagnia del Tralcio, envoyé pour intimider ou éliminer les gêneurs de son commerce."
---

## Actions

- **Attaques multiples.** Deux attaques à la dague.
- **Dague.** Attaque au corps à corps ou à distance, +5, 1,5/6/18 m ; 5 (1d4+3) dégâts perforants, +7 (2d6) dégâts si la cible est surprise.
- **Poison paralysant (objet, 1 dose).** La cible touchée doit réussir un jet de Constitution (DD 13) ou être Paralysée pendant 1 minute (nouveau jet à la fin de chaque tour).

## Notes de MJ

Utilisé par Ilaria Conti pour intimider témoins ou négociateurs (Arc I) ; sa capture vivant peut fournir une preuve décisive contre la Compagnia del Tralcio.